document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateMediaSessionMetadata(track) {
    ...
    const artworkPath = (data.albumFolder || '') + (data.cover || '');
    const artworkSrc = artworkPath.startsWith('http') ? artworkPath : baseUrl + artworkPath;
    const meta = {
      title: track.title || '',
      artist: data.artist || 'EchoPress Artist',
  ...
  function updateMediaSessionPosition() {
    if (
      ...
      try {
        ...
        // Some browsers may throw if position state is invalid
      }
    }
  }
  ...
  function refreshAllMarquees() {
    ...
    trackListEl.querySelectorAll("li").forEach((li) => {
      ...
      if ((isActive || isHovered) && titleEl.scrollWidth > titleEl.clientWidth) {
        ...
        // active or hovered but no overflow
        disableMarquee(titleEl);
      }
    });
  }
  ...
  // Extras (album info / lyrics)
  const extrasTabs = playerContainer.querySelectorAll(
    ".tab-btn:not(.close-tab-btn)"
  );
  const closeTabBtn = playerContainer.querySelector(".close-tab-btn");
  const infoView = playerContainer.querySelector("#info-view");
  ...
  // Cache for preloaded tracks to avoid re-downloading
  ...
  function loadTrack(idx) {
    ...
    // ✅ Robust word/track key (handles string numbers, blanks, etc.)
    const num = Number.isFinite(Number(track.number)) ? Number(track.number) : (idx + 1);
    const key = `track${String(num).padStart(2, '0')}`;
    audioEl.setAttribute("data-track-key", key); // ← animation reads this

    // 1) Audio source + track-info
    const cached = trackCache.get(track.file);
    audioEl.src = cached ? cached : data.albumFolder + track.file;
    ...
    // Preload the upcoming track once the current one is fully buffered
    if (cached) {
      preloadNextTrack();
    } else {
      const onProgress = () => {
        try {
          if (audioEl.buffered.length && audioEl.buffered.end(0) >= audioEl.duration) {
  ...
  function preloadNextTrack() {
    ...
    if (trackCache.has(nextTrack.file)) return;
    fetch(data.albumFolder + nextTrack.file)
      .then(res => res.blob())
      .then(blob => {
        const url = URL.createObjectURL(blob);
        trackCache.set(nextTrack.file, url);
      })
  ...
  function showTrackLoading() {
    const li = trackListEl.querySelector(`li[data-index="${currentIndex}"]`);
    if (!li) return;
    const numberEl = li.querySelector('.title-number');
    if (numberEl) {
      numberEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
    }
    li.classList.add('loading');
    if (trackInfoTextEl) {
      trackInfoTextEl.textContent = 'Loading…';
    }
  }

  function scheduleTrackLoading() {
    cancelTrackLoading();
    loadingTimeout = setTimeout(() => {
      if (
        audioEl.readyState < HTMLMediaElement.HAVE_FUTURE_DATA
      ) {
        showTrackLoading();
      }
      loadingTimeout = null;
  ...
  function restoreTrackNumber() {
    ...
    if (numberEl) {
      const track = tracks[currentIndex];
      const num = track.number || currentIndex + 1;
  ...
  audioEl.addEventListener('waiting', scheduleTrackLoading);
  ...
  audioEl.addEventListener('canplaythrough', () => {
    if (pendingPlay && audioEl.paused) {
      audioEl.play().then(() => { pendingPlay = false; }).catch(() => {});
    }
  });
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener("click", () => {
      const infoMode = !playerContent.classList.contains("info-mode");
      playerContent.classList.toggle("info-mode", infoMode);
      if (toggleIcon) {
        toggleIcon.classList.toggle("fa-list", infoMode);
        toggleIcon.classList.toggle("fa-info-circle", !infoMode);
      }
      toggleBtn.title = infoMode ? "Show Tracklist" : "Show Album Info";
    });
  }

  // Extras Tabs (Album Info / Lyrics)
  if (extrasTabs.length && infoView && lyricsView) {
  ...
  if (closeTabBtn) {
    closeTabBtn.addEventListener("click", () => {
      if (playerContent.classList.contains("info-mode")) {
        playerContent.classList.remove("info-mode");
        if (toggleIcon) {
          toggleIcon.classList.remove("fa-list");
          toggleIcon.classList.add("fa-info-circle");
        }
        if (toggleBtn) toggleBtn.title = "Show Album Info";
      }
    });
  }
  ...
  window.addEventListener("refresh-marquees", refreshAllMarquees);
  ...
  const hasBackCover = window.albumData && window.albumData.hasBackCover;
  if (!coverInner || !hasBackCover) {
    if (flipButton) flipButton.style.display = "none";
    return; // nothing to do if there’s no cover or no back cover
  }
  ...
  document.addEventListener('click', () => {
    downloadMenus.forEach((menu) => menu.classList.remove('open'));
  });
